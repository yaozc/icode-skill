...
**与后续步骤的关系**: **独立步骤，不自动串联到步骤1**。完成后用户须显式运行 `$icodex start`（标准全流程）/ `$icodex fast`（精简全流程）/ `$icodex plan`（仅步骤1）才进入步骤1。复用规则详见 SKILL.md「调用命令」段的目录复用规则说明。
...
9. **工作量评估 + 入口建议**（自动给用户判断，省"我也不知道该 start 还是 fast"的纠结）：
   ...
   - **入口建议**（按评估等级自动给）：
     ...
     - `medium` → 推荐 `$icodex start`（3 轮对抗保险）
     - `large` → **必须** `$icodex start`（多轮对抗 + 完整 deepcheck，跨模块/重构场景）
   - **输出建议块**（替换原"提示用户"语，格式固定）：
     ...
     💡 建议入口：$icodex {start|fast}
     - $icodex start：完整 3 轮对抗 + 完整 deepcheck（{适用情况}）
...
> 本文档由 `$icodex init` 生成，随对话增量更新。运行 `$icodex start` 或 `$icodex plan` 可基于本文档进入步骤1。
...
- **不自动串联**：即使讨论已经非常充分，也不主动跳到步骤1。等用户显式运行 `$icodex start` / `$icodex plan`
...
`$icodex start` / `$icodex plan` 启动时，如果检测到**当前最新目录只有 `00_init.md`（仅含 `.ico_metadata.json` + `00_init.md`，无 `01_plan.md` 等其他步骤产物）**，则：
