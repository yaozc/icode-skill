...
**Codex 调用**: `$icodex init [<粗略需求>]`（兼容：`$icodex stage=00_init`）
**产出**: `{ICODE_OUT_DIR}/00_init.md`
**会话**: 主会话
**与后续步骤的关系**: **独立步骤，不自动串联到步骤1**。完成后用户须显式运行 `$icodex plan` 才进入步骤1。

## 关键约定（必读）

- **`init` 即"新开一次需求初稿讨论"**：每次调用 `$icodex init` 都**创建一个全新的 `.ai/icode/{timestamp}-{short-task}/` 目录**，**不复用**之前任何 `init_in_progress` 状态的目录、**不续聊**之前的讨论。如果用户想继续上一次讨论，就直接对话，**不要再次调用 `init`**。
- **后续讨论由 AI 自主识别并增量更新**：`init` 之后，用户在同一会话里继续提问/补充需求，AI 必须自主判断"这是对当前 `00_init.md` 的补充"，并按"后续每轮对话"流程处理（先 Read，再讨论，最后 Write 更新文档）。无需用户每轮都敲命令。
...
2. 处理输入参数（**两种都支持**）：
   - **有参数**（`$icodex init <粗略需求>`）：将参数作为初始需求，先基于参数结合项目代码做一轮初步分析，产出第一版 `00_init.md`
   - **无参数**（`$icodex init`）：产出空模板版 `00_init.md`（各章节内容写"待补"），然后主动询问用户"这次想做什么？"开启对话
...
7. 提示用户：可继续对话补充需求，文档会随对话自动更新；讨论完成后运行 `$icodex plan` 进入步骤1。**若想另起炉灶讨论别的需求**，再调用 `$icodex init`（会新建另一个目录）。
...
> 本文档由 `$icodex init` 生成，随对话增量更新。运行 `$icodex plan` 可基于本文档进入步骤1。
...
- **不自动串联**：即使讨论已经非常充分，也不主动跳到步骤1。等用户显式运行 `$icodex plan`
- **保持模板结构**：无论信息多少，5 个章节都要存在，不足处写"待补"
- **status 维持 `init_in_progress`**：每次更新文档时，metadata 的 `status` 保持 `init_in_progress`（等步骤1启动消费时，由步骤1切换）

## 与步骤1的衔接

`$icodex plan` 启动时，如果检测到**当前运行目录只有 `00_init.md`（仅含 `.ico_metadata.json` + `00_init.md`，无 `01_plan.md` 等其他步骤产物）**，则：
