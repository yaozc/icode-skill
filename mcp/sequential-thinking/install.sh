#!/usr/bin/env bash
# sequential-thinking 一键注册 MCP server 到 ~/.claude.json
# 使用:
#   ./install.sh                  # 注册默认包 @modelcontextprotocol/server-sequential-thinking
...
if [ ! -f "$HERE/scripts/register_mcp.py" ]; then
  echo "❌ 当前目录不是 sequential-thinking 工程根目录"
  echo "   source: $HERE"
  echo "   请 cd 到 mcp/sequential-thinking 目录后重跑"
  exit 1
fi
...
echo "📦 sequential-thinking 安装"
echo "   node: $NODE_BIN"
echo "   npx:  $NPX_BIN"
echo "   pkg:  ${1:-@modelcontextprotocol/server-sequential-thinking}"
