#!/usr/bin/env bash
# playwright 一键注册 MCP server 到 ~/.claude.json
# 使用:
#   ./install.sh                  # 注册默认包 @playwright/mcp
...
if [ ! -f "$HERE/scripts/register_mcp.py" ]; then
  echo "❌ 当前目录不是 playwright 工程根目录"
  echo "   source: $HERE"
  echo "   请 cd 到 mcp/playwright 目录后重跑"
  exit 1
fi
