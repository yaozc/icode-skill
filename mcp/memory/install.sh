#!/usr/bin/env bash
# memory 一键注册 MCP server 到 ~/.claude.json
# 使用:
#   ./install.sh                  # 注册默认包 @modelcontextprotocol/server-memory
...
if [ ! -f "$HERE/scripts/register_mcp.py" ]; then
  echo "❌ 当前目录不是 memory 工程根目录"
  echo "   source: $HERE"
  echo "   请 cd 到 mcp/memory 目录后重跑"
  exit 1
fi
...
echo "   node: $NODE_BIN"
echo "   npx:  $NPX_BIN"
echo "   pkg:  ${1:-@modelcontextprotocol/server-memory}"
